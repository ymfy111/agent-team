# TASKFLOW-GOVERNANCE v0.9.26｜恢复记录最小实现
...
真实任务中，节点验证失败时不能为了推进进度而强行标记为 `done`。

### 最小规则

- `validation != PASS` 且未指定状态时，节点进入 `needs_review`；
- 显式 `--status blocked` 时，节点进入 `blocked`；
- `needs_review / blocked` 均不计入完成进度；
- `validate-statuses` 必须能识别 `FAIL` 却 `done / accepted` 的非法状态；
- 本版本不实现恢复命令和完整状态机。


## v0.9.26 恢复记录最小实现

TF-GF-IMPL-04 增加恢复记录最小能力：

- `append-event`：向 `TASKFLOW:EVENTS` 追加结构化 `TaskEvent`，用于记录恢复依据、补充说明、交接、人工确认等轻量事件。
- `resume-node`：允许节点从 `needs_review / blocked / paused / in_progress` 等可恢复状态回到 `in_progress`，并追加 `NODE_RESUMED` 事件。
- 恢复事件必须记录 `reason`，建议记录 `source` 与 `evidenceRefs`。
- P0 阶段不引入完整状态机、任务锁、Runtime 自动调度或 UI，只保证恢复过程可追踪。
- `resume-node` 不应绕过依赖检查、Blocker/Decision 检查和证据要求。

本能力对应智能软件工厂产品层的 `TaskEvent` 与 `TaskTicket` 状态恢复记录；复杂项目后续可由平台 UI / Runner 将该事件流产品化。
