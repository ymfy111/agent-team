# TASKFLOW-GOVERNANCE v0.9.21｜TaskTicket 实际计时口径
...
v0.9.21 已将口径收敛为 TaskTicket 实际时间字段：`start-node` 写实际开始时间，`complete-node` 通过完成门禁后写实际完成时间，并自动计算实际耗时。
...
## v0.9.20 补充：可信耗时证据落盘校验（已被 v0.9.21 收敛）
...
2. v0.9.21 后，`complete-node` 默认检查 evidence ref 对应文件的 mtime；若证据缺失或 mtime 晚于 `actualCompletedAt`，拒绝完成。
...
## v0.9.21 收敛：TaskTicket 实际开始/完成时间
